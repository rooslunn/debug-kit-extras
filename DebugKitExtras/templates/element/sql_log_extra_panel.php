...
use DebugKitExtras\Panel\SqlLogExtraPanel;
...
<?php if (!empty($loggers)) : ?>
    <?php foreach ($loggers as $logger) : ?>
        ...
        $highlightOpacity = Configure::read('SqlLogExtra.highlightOpacity', SqlLogExtraPanel::HIGHLIGHT_OPACITY);
        $noOutput = false;
        ?>
            ...
            <table cellspacing="0" cellpadding="0">
                ...
                <tbody>
                    <?php foreach ($queries as $query) : ?>
                        <?php $query['color'] = substr(hash('md5', $query['query']), -6) ?>
                        <tr style="background: #<?= $query['color'] . $highlightOpacity ?>">
